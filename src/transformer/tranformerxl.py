...
def train(model, num_epochs=50, num_segments=600):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            loss_group = torch.tensor(0.0).to(device)
            ...
            loss_group = torch.divide(loss_group, group_size)
            if device.type == 'cuda':
                epoch_train_losses.append(loss_group.cpu().detach().numpy())
            else:
                epoch_train_losses.append(loss_group.detach().numpy())
            print('>>> Epoch: {}, Group loss: {:.5f}, Time: {:.2f} min'.format(epoch, loss_group, (time.time() - st) / 60))
...
def generate(model, n_target_bar, temperature, topk, output_path, prompt=None, counter=0):
    ...
    while current_generated_bar < n_target_bar:
        ...
        if len(words[0]) % 200 == 0 and generation_idx != 0:
            words[0].append(event2word['Bar_None'])
            current_generated_bar += 1
