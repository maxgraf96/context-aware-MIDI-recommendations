...
def generate_from_midi_file(path):
    """
    Generate a bar of MIDI from another MIDI file and save it to './result/from_midi_file.mid'
    :param path: The path of the original midi file
    :return: None
    """
    print("Generating from existing midi file at ", str(path))

    # Output path of the three new midi files
    out_path = 'result/from_midi_file.mid'

    # generate midi file from conditioned => overwrite generated for now
    generate(model,
             n_target_bar=1,
             temperature=1.2,
             topk=5,
